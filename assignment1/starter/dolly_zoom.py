...
def dolly_zoom(
    ...
    initial_d = 3.9
    initial_fov = torch.tensor(60.0)
    h = initial_d * torch.tan(torch.deg2rad(initial_fov / 2))

    renders = []
    for fov in tqdm(fovs):
        distances = h / torch.tan(torch.deg2rad(fov / 2))
        T = [[0, 0, distances]]  # TODO: Change this.
